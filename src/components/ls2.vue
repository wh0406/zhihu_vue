...
<template>
  <div>
    <div class="left_content_bottom" v-for="(item, key) in list2" :key="key">
      <div>
        <ul class="firstul">
        ...
        <ul class="secondul">
          ...
          <li>
            <button class="btncomm" @click="islovebtn(key + 1)">
  ...
  methods: {
    ...
    islovebtn(key) {
      if (this.list[key].islove == false) {
        this.$axios.patch("http://localhost:3000/list/" + key, {
          islove: true,
        });
        this.init();
      } else {
    ...
    //添加评论
    pushcommlist(key) {
      this.list[key].commentlist.push({
